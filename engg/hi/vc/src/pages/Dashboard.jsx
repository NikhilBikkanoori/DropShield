...
const Dashboard = () => {
  ...
  // Generate random risk value on component mount
  const riskData = useMemo(() => {
    const randomRisk = Math.floor(Math.random() * 100) + 1; // 1-100
    ...
    if (randomRisk < 45) {
      // High Risk - Low GPA and Low Attendance
      riskType = 'High Risk';
      riskColor = '#FF6B6B'; // Red
      gpa = (Math.random() * 2 + 2).toFixed(1); // 2.0-4.0
      attendance = Math.floor(Math.random() * 30 + 30); // 30-60%
    } else if (randomRisk >= 45 && randomRisk <= 80) {
      // Medium Risk - Medium GPA and Medium Attendance
      riskType = 'Medium Risk';
      riskColor = '#FFD166'; // Yellow/Orange
      gpa = (Math.random() * 1.5 + 3).toFixed(1); // 3.0-4.5
      attendance = Math.floor(Math.random() * 25 + 65); // 65-90%
    } else {
      // Low Risk - High GPA and High Attendance
      riskType = 'Low Risk';
      riskColor = '#06D6A0'; // Green
      gpa = (Math.random() * 0.5 + 3.5).toFixed(1); // 3.5-4.0
      attendance = Math.floor(Math.random() * 10 + 90); // 90-100%
    }

    return {
      percentage: randomRisk,
      type: riskType,
      color: riskColor,
      gpa: gpa,
      attendance: attendance,
      trend: Array.from({ length: 8 }, () => Math.floor(Math.random() * 20) + (randomRisk - 10))
    };
  }, []);
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <div className="container">
        ...
        <aside className="sidebar" id="sidebar">
          ...
          <button onClick={() => setShowHelpModal(true)} style={{ background: 'none', border: 'none', padding: '12px', borderRadius: '10px', marginBottom: '8px', color: 'var(--risk-high)', cursor: 'pointer', textAlign: 'left', width: '100%', fontWeight: 600 }}>🆘 Get Help</button>

          <div style={{ marginTop: 'auto', paddingTop: '20px' }}></div>
          <button onClick={() => navigate('/')} style={{ background: 'rgba(162,244,249,0.1)', border: '1px solid rgba(162,244,249,0.2)', padding: '12px', borderRadius: '10px', color: 'var(--c4)', cursor: 'pointer', textAlign: 'left', width: '100%', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 600 }}>
        ...
        {activePage === 'dashboardPage' && (
          <main className="main" id="dashboardPage">
            <div className="dashboard-content">
              ...
              <div className="risk-overview">
                <div className="risk-circle">
                  <div className="risk-circle-inner">
                    <div className="risk-percentage">{riskData.percentage}%</div>
                    <div className="risk-label">Risk Score</div>
                  </div>
                </div>
                <div className="risk-status" style={{ color: riskData.color }}>{riskData.type}</div>
              ...
              <div className="dashboard-section" style={{ padding: '0 20px 20px' }}>
                <h3 style={{ marginBottom: '15px' }}>7-Day Risk Trend</h3>
                <div className="card" style={{ background: 'rgba(255,255,255,0.03)', padding: '20px' }}>
                  <RiskTrendChart data={riskData.trend} color={riskData.color} />
                </div>
              </div>
        ...
        {activePage === 'marksPage' && (
          <main className="main" id="marksPage">
            <div className="dashboard-content">
              ...
              <div className="dashboard-section">
                <p>Current GPA: <strong style={{ color: 'var(--c4)' }}>{riskData.gpa}</strong></p>
              </div>
              <div className="dashboard-section">
                <table>
                  <thead>
        ...
        {activePage === 'attendancePage' && (
          <main className="main" id="attendancePage">
            <div className="dashboard-content">
              ...
              <div className="dashboard-section">
                <p>Current Attendance: <strong style={{ color: 'var(--c4)' }}>{riskData.attendance}%</strong></p>
                <div className="progress-bar">
                  <div className="progress-fill" style={{ width: `${riskData.attendance}%`, background: 'var(--c4)' }}></div>
                </div>
              </div>
            </div>
          </main>
