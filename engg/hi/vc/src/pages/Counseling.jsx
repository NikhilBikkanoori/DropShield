...
import { COUNSELING_STUDENT_DATA, INITIAL_ACTIONS_DATA } from "../data/mockData";
...
const Counseling = () => {
  ...
  const [actions, setActions] = useState(INITIAL_ACTIONS_DATA);
  ...
  const studentData = COUNSELING_STUDENT_DATA;
    ...
    <div style={{ background: '#192047', minHeight: '100vh', color: '#F7FAFC', fontFamily: 'Inter, sans-serif' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; }
        ...
        @media(max-width:900px){
          ...
          aside.sidebar { width: 100%; display: flex; overflow-x: auto; max-height: unset; white-space: nowrap; gap: 10px; padding: 10px; }
          .nav-item { width: auto; flex-shrink: 0; padding: 8px 12px; margin: 0; }
          aside.sidebar h2, aside.sidebar hr, aside.sidebar > div { display: none; }
          aside.sidebar > button.primary, aside.sidebar > button:last-child { display: none; } /* Hide heavy sidebar stuff on mobile */
          .grid > section { grid-column: 1 / -1 !important; }
          ...
          .top-right button { display: inline-block; }
        }
        @media(max-width:500px){
          .grid[style*="1fr 1fr !important"] { grid-template-columns: 1fr !important; }
        }
      `}
      </style>
      ...
      <div className="container">
        ...
        <aside className="sidebar">
          ...
          <button className="btn primary" style={{ width: '100%', marginTop: '12px', padding: '12px', background: 'rgba(245,158,11,0.2)', color: '#f59e0b', border: '1px solid rgba(245,158,11,0.4)' }}>Request Immediate Help</button>

          <div style={{ marginTop: 'auto', paddingTop: '20px' }}></div>
          <button onClick={() => navigate('/dashboard')} style={{ background: 'rgba(162,244,249,0.1)', border: '1px solid rgba(162,244,249,0.2)', padding: '12px', borderRadius: '10px', color: 'var(--c4)', cursor: 'pointer', textAlign: 'left', width: '100%', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 600 }}>
            ← Back to Dashboard
          </button>
        </aside>
